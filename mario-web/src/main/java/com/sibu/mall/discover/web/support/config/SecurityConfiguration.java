...
import com.mall.discover.web.support.filter.TokenUidSubject;
...
public class SecurityConfiguration implements SecurityFilterConfigurer {
    ...
    public void addSecurityFilterChainDefinition(SecurityFilterChainDefinitionRegistry configurer) {
        ...
        configurer.addSecurityFilterChainDefinition("optionAuthc").addPathPatterns(getOptionAuthcPathPatterns()).order(1);
        //默认所有都必须验证通过才可以访问
    ...
    public static class OptionAuthcFilter extends AuthenticationFilter {
        ...
        @Override
        protected boolean isAccessAllowed(ServletRequest request, ServletResponse response, Object mappedValue) {
            TokenUidSubject subject = (TokenUidSubject)this.getSubject(this.getHttpServletRequest(request), this.getHttpServletResponse(response));

            //游离状态，token 可选（不为空则验证）
            return subject != null && (StringUtil.isBlank(subject.getToken()) || subject.isAuthenticated());
        }

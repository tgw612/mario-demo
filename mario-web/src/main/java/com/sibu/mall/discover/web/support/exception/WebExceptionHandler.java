        ...
        if (cause != null) {
            ...
            if (cause instanceof RemotingException){
                ...
                if(remoteExceMsg != null && remoteExceMsg.startsWith("com.alibaba.csp.sentinel.slots.block.SentinelRpcException")){
                    log.error("[{}] RPC SentinelRpcException remoteAddr:[{}]", SerialNo.getSerialNo(), remoteAddr);
//                    return SentinelRpcExceptionResolver.handle((SentinelRpcException) cause);
        ...
        //TODO
        log.error("[{}] RPC Exception Occur:[{}]", SerialNo.getSerialNo(), ExceptionUtil.getAsString(ex));
...
//    @ResponseBody
//    @ResponseStatus(value = HttpStatus.OK)
    private CommonResponse handleAllException(Throwable ex/*, HttpServletRequest req, HttpServletResponse res*/) {

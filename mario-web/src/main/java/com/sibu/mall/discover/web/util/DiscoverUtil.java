...
public class DiscoverUtil {
     ...
     * @param userIds
     */
    public static List<ClientLikePageResponse> getUserInfoList(UserService userService, List<Integer> userIds) {
        ...
        CommonResponse<List<UserShowResponse>> userShowInfoByIds = userService.findUserShowInfoByIds(userRequest);
    ...
    public static List<ClientLikePageResponse> sortUserInfoList(List<Integer> userIds, List<ClientLikePageResponse> userInfoList) {
        List<ClientLikePageResponse> result = new ArrayList<>();
     ...
     * @param result
     */
    public static void getUserLikePage(Cache<Long, LinkedHashSet<Integer>> articleLikeUserListCache, List<ClientArticleResponse> result) {
        if (ObjectUtils.isEmpty(result)) {
            return;
        }
        //获取用户id
        Integer userId = AccessTokenUtil.getCurrentUserId();
        Set<Long> articleIds = result.stream().map(ClientArticleResponse::getArticleId).collect(Collectors.toSet());
    ...
    public static String getReadCountString(Integer likeCount) {
        ...
        BigDecimal result = new BigDecimal(likeCount).divide(new BigDecimal(divide), 1, BigDecimal.ROUND_HALF_UP);
     ...
     * @return
     */
    public static Long getProductId(Long productId, String productNo, Cache<String, Long> productNoIdRelation, ClientProductService clientProductService) {
        if (productId == null || productId <= 0) {
            ...
            if (ObjectUtils.isEmpty(resultId)) {
                CommonResponse<Long> longCommonResponse = clientProductService.queryProductNoIdRelation(productNo);

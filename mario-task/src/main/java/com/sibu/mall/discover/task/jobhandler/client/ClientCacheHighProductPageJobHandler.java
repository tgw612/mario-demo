 ...
 * @author: huangzhong
 * @Date: 2019/10/15
 * @Description: 缓存高佣金商品列表
 */
@JobHandler(value = "clientCacheHighProductPageJobHandler")
@Service
public class ClientCacheHighProductPageJobHandler extends IJobHandler {
    ...
    public ReturnT<String> execute(String s) throws Exception {
        ...
        for (int i = 1; i <= cacheMaxPage; i++) {
            ...
            CommonResponse<CommonPageResult<ClientProductPageResponse>> response = clientProductService.queryHighProductPage(request, true);
            if (!response.isSuccess()) {
                XxlJobLogger.log("调用[ClientProductService][queryHighProductPage]dubbo 接口出错,request:{},response:{}",

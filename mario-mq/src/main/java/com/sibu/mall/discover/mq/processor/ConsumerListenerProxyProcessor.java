    ...
    public Object postProcessBeforeInitialization(Object bean, String beanName) throws BeansException {
        try {
            if (bean instanceof RocketMqConsumerBean) {
                Field mapfield = FieldUtils.getDeclaredField(RocketMqConsumerBean.class, "topicTagListenerMap", true);
                Map<String, Map<String, RocketMqMessageListener>> topicMap = (Map<String, Map<String, RocketMqMessageListener>>) FieldUtils.readField(mapfield, bean, true);
                for (Map<String, RocketMqMessageListener> tagMap : topicMap.values()) {
                    for (String key : tagMap.keySet()) {
                        //原生listener
                        RocketMqMessageListener listener = tagMap.get(key);
                        //代理listener
                        RocketMqMessageListener listenerProxy = (RocketMqMessageListener) new ListenerMethodInterceptor().getInstance(listener);

...
import com.mall.discover.config.QcloudConfigProperties;
import com.mall.discover.config.properties.DiscoverProperties;
import com.mall.discover.mq.VodHandlerMqProducer;
import com.mall.discover.mq.request.ArticleVodUpdateRequest;
...
import com.mall.discover.util.SignatureUtil;
...
//        }
    ...
    private void updateCount(AddOrEditArticleRequest request) {
        ...
        if (ObjectUtil.isNotNull(articleCount.getSort())
            ...
            articleCount.setSort(request.getSort() == null || request.getSort() <= 0 ||
                    request.getSort() > discoverProperties.getMaxSort() ? discoverProperties.getMaxSort() : request.getSort());
    ...
    private void saveCount(Long sort, ArticleEntity entity) {
        ...
        if (ObjectUtil.isNull(existCountEntity)) {
            ...
            articleCountEntity.setSort(sort == null || sort <= 0 || sort > discoverProperties.getMaxSort() ?
            ...
            log.error("articleServiceBiz saveCount保存articleCount报错,数据库已存在数据:" + JsonUtil.toJson(existCountEntity));
    ...
    public Boolean updateArticleLookVod(VodLook vodLook, Long articleId) {
        if (vodLook != null && StringUtil.isNotBlank(vodLook.getCoverUrl())
            ...
            ArticleLook articleLook = JsonUtil.parseObject(articleLookStr, new TypeReference<ArticleLook>() {
            });
    ...
    public CommonResponse<CommonPageResult<ArticleListResponse>> listArticle(ListArticleRequest request) {
        try {
            ...
            List<ArticleListBO> articleListBOList = articleMapper.listArticle(bo, PageUtils.getStartPage(request), request.getPageSize());
            ...
            List<ArticleListResponse> list = JsonUtil.parseObject(JsonUtil.toJSON(articleListBOList).toString(),
            ...
            log.error("[{}]分页文章查询动态失败, Exception:{}", SerialNo.getSerialNo(), ExceptionUtil.getAsString(e));
            return ResponseManage.fail("分页查询动态失败");
        }
    }

    private void setProduct(ArticleListBO articleListBO) {
        List<ProductIdNameBO> productIdNameBOList = articleMapper.getArticleRelateProduct(articleListBO.getArticleId(),
    ...
    private void setSubject(ArticleListBO articleListBO) {
        List<SubjectInfoBO> relateSubjectName = articleMapper.getRelateSubjectName(articleListBO.getArticleId(),
    ...
    private void setPictureNums(ArticleListBO articleListBO) {
        ArticleLook look = JsonUtil.parseObject(articleListBO.getArticleLook(), new TypeReference<ArticleLook>() {
        });
    ...
    private void setOrderField(ListArticleRequest request) {
        if (ObjectUtil.isNotNull(request.getSortField()) && ObjectUtil.isNotNull(request.getSortOrder())) {
    ...
    public CommonResponse<ArticleInfoResponse> getArticleInfo(Long articleId) {
        try {
            ...
            List<SubjectInfoBO> subject = articleMapper.getRelateSubjectName(articleId, RelationTypeEnum.SUBJECT_ARTICLE.getCode());
            info.setSubject(subject);
            ArticleInfoResponse result = BeanCopyUtils.copyProperties(ArticleInfoResponse.class, info);
            result.setPictureUrls(JsonUtil.parseObject(info.getArticleLook(), new TypeReference<ArticleLook>() {
            }));
            ...
            productIds.forEach(productId -> {
                ProductInfoResponse infoResponse = productServiceBiz.queryProductInfo(productDao.searchNoByID(productId));
    ...
    public CommonResponse<CommonPageResult<DiscoverCountListResponse>> listDiscoverCount(ListDiscoverCountRequest request) {
        try {
            ...
            List<DiscoverCountListResponse> data = BeanCopyUtils.copyList(DiscoverCountListResponse.class, list);
            result.setData(data);
            result.setTotalCount(count);
            result.setTotalPage(PageUtils.getPageCount(request.getPageSize(), count));
            return ResponseManage.success(result);
        } catch (Exception e) {
            log.error("[{}]分页discovercount查询动态失败, Exception:{}", SerialNo.getSerialNo(), ExceptionUtil.getAsString(e));
    ...
    public CommonResponse<Boolean> editArticleSortClient(EditArticleSortClientRequest request) {
        try {
            List<DiscoverCountBO> data = BeanCopyUtils.copyList(DiscoverCountBO.class, request.getBachList());
            articleCountMapper.editDiscoverCountSortClient(data);
            return ResponseManage.success(Boolean.TRUE);
        } catch (Exception e) {
            log.error("[{}]修改文章client_sort失败, Exception:{}", SerialNo.getSerialNo(), ExceptionUtil.getAsString(e));
    ...
    public CommonResponse<Boolean> updateReadShareCount() {
        ...
        for (int i = 1; i <= totalPage; i++) {
            ...
            try {
                ...
                log.error("[{}]定时更新阅读数分享数出错, Exception:{}", SerialNo.getSerialNo(), ExceptionUtil.getAsString(e) + JsonUtils.toJson(updateBOList));
    ...
    private void getCacheCount(List<CountReadShareUpdateBO> updateBOList, CountReadShareUpdateBO obj) {
        ...
        if (ObjectUtil.isNotNull(obj.getReadCount()) && ObjectUtil.isNotNull(obj.getShareCount())) {
            if (!obj.getReadCount().equals(originReadCount) || !obj.getShareCount().equals(originShareCount)) {
    ...
    public CommonResponse<Boolean> publishArticle() {
        //更新话题发布状态
        List<Long> subjectIds = subjectMapper.queryWaitPublishSubjectId(System.currentTimeMillis(), SubjectStatusEnum.TO_PUBLISHED.getCode());
        if (!ObjectUtils.isEmpty(subjectIds)) {
            subjectMapper.updateWaitPublishSubStatus(System.currentTimeMillis(), SubjectStatusEnum.PUBLISHED.getCode(), subjectIds);
        }
...
//        }
    ...
    private void updateArticleLook(EventContent eventContent, VodLook vodLook) {
        Long articleId = vodIdArticleIdCache.get(eventContent.getProcedureStateChangeEvent().getFileId());
        if (ObjectUtil.isNull(articleId)) {
            articleId = articleMapper.selectLikeArticleLook(eventContent.getProcedureStateChangeEvent().getFileId());
            if (articleId == null) {
                confirmEvent(eventContent);
            }
        }
...
//            eventContent.getProcedureStateChangeEvent().
    ...
    private VodLook generateVodLook(EventContent eventContent) {
        MediaProcessTaskResult[] medias = eventContent.getProcedureStateChangeEvent().getMediaProcessResultSet();
        ...
        for (MediaProcessTaskResult media : medias) {
            if ("Transcode".equals(media.getType())) {
                ...
                if (!ObjectUtil.isNull(vodLook.getWidth()) && ObjectUtil.isNotNull(vodLook.getHeight())) {
                    vodLook.setWhRatio(new BigDecimal(vodLook.getWidth()).divide(new BigDecimal(vodLook.getHeight()), 2));
                }
            }

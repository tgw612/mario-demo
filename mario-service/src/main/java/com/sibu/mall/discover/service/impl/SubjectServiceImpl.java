...
import com.mall.discover.service.impl.biz.SubjectBiz;
...
public class SubjectServiceImpl implements SubjectService {
    ...
    public CommonResponse<Boolean> subjectSubmit(SubjectSubmitRequestDto request) {
        if (request.getSubjectId() != null && request.getSubjectId() > 0) {
            ...
            if (publishTime != null && !publishTime.equals(request.getPublishTime()) && countSubjectArticle > 0) {
    ...
    @Override
    public CommonResponse<CommonPageResult<SubjectResponse>> querySubjectPage(SubjectPageRequestDto request) {
        ...
        if (subjectCommonPageResult.getData().size() > 0) {
            ...
            for (SubjectBO bo : bolist) {
                ...
                try {
                    subjectResponse.setSubjectLook(objectMapper.readValue(bo.getSubjectLook(), SubjectLook.class));
                } catch (IOException e) {
                    e.printStackTrace();
//                    throw new BusinessException("返回值转换失败");
    ...
    @Override
    public CommonResponse<CommonPageResult<SubjectArticleResponse>> querySubjectArticlePage(SubjectArticlePageRequestDto request) {
        CommonPageResult<SubjectArticleResponse> subjectArticleResponseCommonPageResult = subjectBiz.querySubjectArticlePage(request);
    ...
    @Override
    public CommonResponse<CommonPageResult<SubjectNameResponse>> querySubjectByName(SubjectNameRequestDto request) {

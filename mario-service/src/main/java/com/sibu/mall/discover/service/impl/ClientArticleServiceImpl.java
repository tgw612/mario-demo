...
import com.mall.discover.service.impl.biz.ClientArticleBiz;
import com.mall.discover.service.impl.biz.ClientProductBiz;
import com.mall.discover.service.impl.biz.ClientSubjectBiz;
...
public class ClientArticleServiceImpl implements ClientArticleService {
    ...
    @Reference(consumer = DubboConstants.SIBU_MALL_DARWIN_CONSUMER)
    ...
    @Override
    public CommonResponse<CommonPageResult<ClientDiscoverResponse>> queryArticlePage(ClientArticlePageRequest request, boolean cache) {
        ...
        articleList.forEach(articleBO -> articleResponseList.add(DiscoverUtil.getClientArticleResponse(articleBO)));
        ...
        if (cache && responsesList.size() > 0 && request.getCurrentPage() < clientCacheInfoMaxPage) {
            ...
            for (ClientDiscoverResponse response : responsesList) {
                ...
                if (response instanceof ClientArticleResponse) {
                    //文章详情
                    ClientArticleInfoResponse infoResponse = DiscoverUtil.convertResponse(response, ClientArticleInfoResponse.class);
                    //图片音频集合
                    ...
                    articleQueryInfo.put(infoResponse.getArticleId(), infoResponse, clientExpireTime, TimeUnit.SECONDS);
                }
                ...
                if (response instanceof ClientSubjectResponse) {
                    ...
                    subjectQueryInfo.put(clientSubjectResponse.getSubjectId(), infoResponse, clientExpireTime, TimeUnit.SECONDS);
                }
            }
        }
    ...
    @Override
    public CommonResponse<ClientArticleInfoResponse> queryArticleInfo(ClientArticleInfoRequest request, boolean cache) {
        ...
        if (articleBO != null) {
            ...
            //缓存
            articleQueryInfo.put(result.getArticleId(), result, discoverProperties.getClientExpireTime(), TimeUnit.SECONDS);
        }
        return ResponseManage.success(result);
    }

    @Override
    public CommonResponse<CommonPageResult<ClientDiscoverResponse>> queryArticlePageOld(ClientArticlePageRequest request, boolean cache) {
        //全部缓存
        CommonPageResult<ClientDiscoverResponse> result = new CommonPageResult<>();
        List<ClientDiscoverResponse> responsesList = new ArrayList<>();
        ...
        for (ArticleBO articleBO : articleList) {
            ClientArticleResponse clientArticleResponse = DiscoverUtil.getClientArticleResponseOld(articleBO);
            //防止错误数据
        ...
        if (cache && request.getCurrentPage() < discoverProperties.getClientCacheInfoMaxPage()) {
            ...
            for (ClientDiscoverResponse response : responsesList) {
                ...
                if (response instanceof ClientArticleResponse) {
                    ...
                    articleQueryInfoOld.put(infoResponse.getArticleId(), infoResponse, clientExpireTime, TimeUnit.SECONDS);
                }
            }
        }
        return ResponseManage.success(result);
    ...
    @Override
    public CommonResponse<ClientArticleInfoResponse> queryArticleInfoOld(ClientArticleInfoRequest request, boolean cache) {
        ClientArticleInfoResponse result = null;
        ArticleBO articleBO = clientArticleBiz.queryArticleInfo(request);
        if (articleBO != null) {
            result = new ClientArticleInfoResponse();
            ...
            //缓存
            articleQueryInfoOld.put(result.getArticleId(), result, discoverProperties.getClientExpireTime(), TimeUnit.SECONDS);
        }
        return ResponseManage.success(result);
    }
     ...
     */
    private List<ClientSubjectResponse> getDataFromDarwin(List<ClientSubjectInfoResponse> subjectInfoResponseList) {
        //返回数据
        List<ClientSubjectResponse> subjectResponseList = new ArrayList<>();
        //T台资源
        ...
        try {
            CommonResponse<LaunchResponse> launchResponseCommonResponse = applicationLaunchService.applicationLaunch(applicationLaunchQuery);
            if (launchResponseCommonResponse.isSuccess()) {
                ...
                if (value != null && value.size() > 0) {
                    value.forEach(map -> {
                        ...
                        Integer resourceSort = Integer.valueOf((String)map.get(discoverProperties.getResourceSortField()));
                        ClientSubjectResponse subjectResponse = getSubjectResponse(subjectId, subjectInfoResponseList);
     ...
     * @return
     */
    private ClientSubjectResponse getSubjectResponse(Long subjectId, List<ClientSubjectInfoResponse> subjectInfoResponseList) {
        ...
        ClientSubjectInfoResponse subjectInfoResponse = DiscoverUtil.convertResponse(subject, ClientSubjectInfoResponse.class);
     ...
     */
    private void getArticleResult(List<ClientDiscoverResponse> responsesList, List<ClientSubjectResponse> subjectResponseList, List<ClientArticleResponse> articleResponseList) {
    ...
    private void getArticleInfo(ClientArticleInfoResponse infoResponse, ArticleBO article) {
        ArticleLook articleLook = DiscoverUtil.convertStringResponse(article.getArticleLook(), ArticleLook.class);
        //计算宽高比例
        ...
        //分享赚总金额
        BigDecimal totalCommission = productList.stream().map(ClientProductResponse::getCommission).reduce(BigDecimal::add).orElse(new BigDecimal(0));
...
//        getThreeArticleInfo(clientRecommendCount, recommendSet);
        ...
        //去除本文章
        List<ArticleBO> resultProduct = recommendSet.stream().filter(a -> !article.getArticleId().equals(a.getArticleId())).collect(Collectors.toList());

        //封装数据
        List<ClientArticleResponse> clientArticleResponses = new ArrayList<>();
        resultProduct.forEach(articleOne -> clientArticleResponses.add(DiscoverUtil.getClientArticleResponse(articleOne)));
    ...
    private void getArticleInfoOld(ClientArticleInfoResponse infoResponse, ArticleBO article) {
        ...
        if (ObjectUtils.isEmpty(productList)) {
            ...
            ClientProductResponse productResponse = DiscoverUtil.convertResponse(productBO, ClientProductResponse.class);
        ...
        //去除本文章
        List<ClientArticleResponse> resultProduct = result.stream().filter(a -> !article.getArticleId().equals(a.getArticleId())).collect(Collectors.toList());

        //封装数据
        infoResponse.setArticleList(resultProduct);
    }

    private void getFirstProductList(List<ClientProductResponse> productList, int clientRecommendCount, Set<ArticleBO> recommendSet) {
        List<Long> productIdList = productList.stream().map(ClientProductResponse::getProductId).collect(Collectors.toList());
        if (productIdList.size() > 0) {
            //查找所有相关一级分类
            Set<Integer> categoryIdFirstList = clientProductBiz.queryCategoryIdFirstPage(productIdList);
            //查找旗下所有商品
            if (categoryIdFirstList.size() > 0) {
                List<Long> productIds = clientProductBiz.queryProductPageByCategoryIdFirst(categoryIdFirstList);
     ...
     *  添加商品相关文章
     */
    private void addAllProductArticle(Long productId, int clientRecommendCount, Set<ArticleBO> recommendList) {

        List<ArticleBO> articleBOS = clientProductBiz.queryProductArticlePage(productId, 0, clientRecommendCount);
        recommendList.addAll(articleBOS);
    }

    private List<ClientProductResponse> getClientProductResponseList(ClientArticleInfoResponse infoResponse) {
    ...
    private void getSecondArticleInfo(List<ClientProductResponse> productList, int clientRecommendCount, Set<ArticleBO> recommendSet, Set<Long> productIdSet) {
        if (recommendSet.size() < clientRecommendCount) {
            //通过二级类别获取相关商品
            Set<Integer> collect = productList.stream().map(ClientProductResponse::getProductCategory).collect(Collectors.toSet());
            first: for (Integer productCategory : collect) {
                int currentPage = 1;
                boolean flag = true;
                while (flag) {
                    List<ProductBO> productBOS = clientProductBiz.queryProductListByCategory(productCategory, currentPage, clientRecommendCount);
    ...
    private void getFirstArticleInfo(List<ClientProductResponse> productList, int clientRecommendCount, Set<ArticleBO> recommendSet) {
        for (ClientProductResponse clientProductResponse : productList) {
            addAllProductArticle(clientProductResponse.getProductId(), clientRecommendCount, recommendSet);
            if (recommendSet.size() >= clientRecommendCount) {
                break;
            }
        }
    ...
    private void getFirstArticleInfoById(List<Long> productIdList, int clientRecommendCount, Set<ArticleBO> recommendSet) {

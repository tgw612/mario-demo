...
public class ClientSubjectServiceImpl implements ClientSubjectService {
    ...
    @Override
    public CommonResponse<CommonPageResult<ClientSubjectPageResponse>> querySubjectPage(ClientSubjectPageRequest request, boolean cache) {
        ...
        if (DiscoverUtil.checkTotalCount(totalCount)) {
            ...
            for (SubjectBO subject : subjectList) {
                ClientSubjectPageResponse response = DiscoverUtil.convertResponse(subject, ClientSubjectPageResponse.class);
                //阅读数转换
            ...
            if (cache && request.getCurrentPage() < clientCacheInfoMaxPage) {
                for (ClientSubjectPageResponse response : responseList) {
                    ClientSubjectInfoResponse infoResponse = DiscoverUtil.convertResponse(response, ClientSubjectInfoResponse.class);
                    subjectQueryInfo.put(infoResponse.getSubjectId(), infoResponse, clientExpireTime, TimeUnit.SECONDS);
                    //缓存话题内文章
    ...
    @Override
    public CommonResponse<ClientSubjectInfoResponse> querySubjectInfo(ClientSubjectInfoRequest request, boolean cache) {
    ...
    public CommonResponse<CommonPageResult<ClientArticleResponse>> querySubjectArticlePage(ClientSubjectArticlePageRequest request) {
        ...
        List<ClientSubjectResponse> clientSubjectResponses = Collections.singletonList(clientSubjectResponse);
        ...
        if (DiscoverUtil.checkTotalCount(totalCount)) {
            ...
            for (SubjectArticleBO subjectArticle : subjectArticles) {
                ClientArticleResponse response = DiscoverUtil.convertResponse(subjectArticle, ClientArticleResponse.class);
                //阅读数转换
                String readCountString = DiscoverUtil.getReadCountString(response.getReadCount());
                response.setViewCountString(readCountString);
                //获取第一张图片信息
                PictureLook pictureLook = DiscoverUtil.getArticleFirstPicture(subjectArticle.getArticleLook());
            ...
            if (!ObjectUtils.isEmpty(subjectArticles)) {
                subjectQueryArtilePage.put(request.getSubjectId() + "-" + request.getCurrentPage(), result, discoverProperties.getClientExpireTime(), TimeUnit.SECONDS);
            }
        }
        return ResponseManage.success(result);
    }
    ...
    @Override
    public CommonResponse<CommonPageResult<ClientArticleResponse>> querySubjectArticlePageOld(ClientSubjectArticlePageRequest request, boolean cache) {
        //兼容老版本 批量缓存
        if (cache) {
     ...
     * @return
     */
    private CommonPageResult<ClientArticleResponse> getClientArticleResponsePageOld(ClientSubjectArticlePageRequest request) {
        CommonPageResult<ClientArticleResponse> result = new CommonPageResult<>();
        ...
        if (DiscoverUtil.checkTotalCount(totalCount)) {
            ...
            for (SubjectArticleBO subjectArticle : subjectArticles) {
                ...
                response.setViewCountString(readCountString);
                //获取第一张图片信息
                PictureLook pictureLook = DiscoverUtil.getArticleFirstPictureOld(subjectArticle.getArticleLook());
                if (pictureLook == null) {
                    continue;
                }
                response.setPictureLook(pictureLook);
            ...
            //缓存话题内文章
            if (!ObjectUtils.isEmpty(subjectArticles)) {
                subjectQueryArtilePageOld.put(request.getSubjectId() + "-" + request.getCurrentPage(), result, discoverProperties.getClientExpireTime(), TimeUnit.SECONDS);
            }
        }
        return result;
    }

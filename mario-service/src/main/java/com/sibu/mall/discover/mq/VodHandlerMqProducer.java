...
public class VodHandlerMqProducer {
    ...
    public void sendVodHandlerTask(ArticleVodUpdateRequest videoTranscodeRequest) {
        RockMqMessage<ArticleVodUpdateRequest> message = new RockMqMessage<>(VodTopicEnum.VOD_HANDLER_TOPIC,
                MqMessageBody.getInstance(videoTranscodeRequest, videoTranscodeRequest.getVodLook().getFileId()));
        boolean sendResult = defaultMqProducer.send(message, t -> JsonUtil.toJsonBytes(t, SerializerFeature.EMPTY));

...
public class MemberDiscoverBiz {
    ...
    public CommonResponse<Boolean> submitDiscover(SubmitDiscoverRequest request) {
        try {
            MemberDiscoverEntity memberDiscoverEntity = BeanCopyUtils.copyProperties(MemberDiscoverEntity.class, request);
    ...
    public CommonResponse<CommonPageResult<MemberPageQueryDiscoverListResponse>> pageDiscover(MemberPageQueryDiscoverListRequest request) {
        try {
            ...
            List<MemberPageQueryDiscoverListResponse> datas = BeanCopyUtils.copyList(MemberPageQueryDiscoverListResponse.class, memberDiscoveraEntities);
            return ResponseManage.success(ConverterUtil.convertToPageResult(datas, count.intValue(), ConverterUtil.calculateTotalPage(request.getPageSize(), count.intValue())));
        } catch (Exception e) {
    ...
    public CommonResponse<CommonPageResult<MemberPageQueryDiscoverListResponse>> adminPageDiscover(AdminPageQueryDiscoverListRequest request) {
        try {
            ...
            List<MemberPageQueryDiscoverListResponse> datas = BeanCopyUtils.copyList(MemberPageQueryDiscoverListResponse.class, list);
            return ResponseManage.success(ConverterUtil.convertToPageResult(datas, count.intValue(), ConverterUtil.calculateTotalPage(request.getPageSize(), count.intValue())));
        } catch (Exception e) {
            log.error("[{}]分页查询动态失败, Exception:{}", SerialNo.getSerialNo(), ExceptionUtil.getAsString(e));
        }
    ...
    public CommonResponse<MemberDiscoverDetailResponse> getDetail(PrimarySeqRequest request) {
        try {
            ...
            MemberDiscoverDetailResponse memberDiscoverDetailResponse = BeanCopyUtils.copyProperties(MemberDiscoverDetailResponse.class, entity);
    ...
    public CommonResponse<Boolean> views(PrimarySeqRequest request) {
        try {
            ...
            log.error("[{}]动态浏览人数增加失败, Exception:{}", SerialNo.getSerialNo(), ExceptionUtil.getAsString(e));
        }
    ...
    public CommonResponse<Boolean> forward(PrimarySeqRequest request) {
        try {
            ...
            if (!StringUtil.isNull(memberDiscoverEntity.getVideo())) {
                ...
                List<DiscoverPictureVo> discoverPictureVos = JSON.parseArray(memberDiscoverEntity.getPictures(), DiscoverPictureVo.class);
                discoverPictureVos.stream().map(DiscoverPictureVo::getProductId).distinct().filter(o -> !StringUtil.isNull(o)).collect(Collectors.toList()).forEach(p -> {
    ...
    public CommonResponse<Boolean> submitCate(SubmitCateRequest request) {
        ...
        try {
            MemberDiscoverCateEntity memberDiscoverCateEntity = BeanCopyUtils.copyProperties(MemberDiscoverCateEntity.class, request);
    ...
    public CommonResponse<List<MemberDiscoverCateListResponse>> listCate(ListCateRequest request) {
        try {
            ...
            List<MemberDiscoverCateListResponse> list = BeanCopyUtils.copyList(MemberDiscoverCateListResponse.class, memberDiscoverCateEntities);

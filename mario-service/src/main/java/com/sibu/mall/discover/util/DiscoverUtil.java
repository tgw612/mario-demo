package com.mall.discover.util;
...
public class DiscoverUtil {
    ...
    public static void computerWhRatio(PictureLook pictureLook) {
        ...
        BigDecimal divide = new BigDecimal(width.toString()).divide(new BigDecimal(height), 2, BigDecimal.ROUND_HALF_UP);
    ...
    public static String computerCommissionRatio(BigDecimal price, BigDecimal commission) {
        ...
        BigDecimal divide = commission.multiply(new BigDecimal(100)).divide(price, 2, BigDecimal.ROUND_HALF_UP);
    ...
    public static VodLook getVodLook(String articleLookStr) {
        //获取第一张图片信息
        ArticleLook articleLook = convertStringResponse(articleLookStr, ArticleLook.class);
        if (articleLook.getVodLookList() != null && !CollectionUtils.isEmpty(articleLook.getVodLookList())) {
    ...
    public static String getSubjectPictureUrl(SubjectBO subject) {
        ...
        SubjectLook subjectLook = DiscoverUtil.convertStringResponse(subject.getSubjectLook(), SubjectLook.class);
        List<PictureLook> pictureLookList = subjectLook.getPictureLookList();
        if (pictureLookList != null && pictureLookList.size() > 0) {
            //话题显示一张图片
            PictureLook pictureLook = pictureLookList.stream().min(Comparator.comparing(PictureLook::getSort)).orElseGet(PictureLook::new);
    ...
    public static String getReadCountString(Long readCount) {
        ...
        BigDecimal result = new BigDecimal(readCount).divide(new BigDecimal(divide), 1, BigDecimal.ROUND_HALF_UP);
    ...
    public static Integer getArticleLookType(ArticleLook articleLook) {
        ...
        //仅图片
        if (!ObjectUtils.isEmpty(articleLook.getPictureLookList()) && ObjectUtils.isEmpty(articleLook.getVodLookList())) {
            return ArticleLookTypeEnum.ONLY_PICTURE.getCode();
        }

        //仅视频
        if (!ObjectUtils.isEmpty(articleLook.getVodLookList()) && ObjectUtils.isEmpty(articleLook.getPictureLookList())) {

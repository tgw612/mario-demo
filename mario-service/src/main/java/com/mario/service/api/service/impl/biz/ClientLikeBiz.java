...
public class ClientLikeBiz {
  ...
  public Boolean updateUserLikeArticle() {
    ...
    for (Long articleId : articleIds) {
      //获取点赞数量
      Long likeCount = RedisTemplateUtil
          .getLong(DiscoverCountEnum.ARTICLE_LIKE_COUNT.getCode() + articleId);
      //获取点赞用户
...
//            if (ObjectUtils.isEmpty(likeCount) || ObjectUtils.isEmpty(cacheUserIds) || likeCount != cacheUserIds.size()) {
      ...
      if (flag) {
        ...
        RedisTemplateUtil
            .setLong(DiscoverCountEnum.ARTICLE_LIKE_COUNT.getCode() + articleId, dbUserIds.size());
      } else {
        //需删除集合
        ...
        if (removeUserIds.size() > 0) {
          clientRelationMapper.batchRemoveByToId(articleId, removeUserIds,
              RelationTypeEnum.USER_LIKE_ARTICLE.getCode());
        }
        //批量新增
  ...
  private void batchAddRelation(Long toId, Integer relationType, List<Integer> addUserIds) {
    ...
    while (addUserIds.size() > page * pageSize) {
      List<Integer> collect = addUserIds.stream().skip(page * pageSize).limit(pageSize)
  ...
  private LinkedHashSet<Integer> getDbAllFromId(Long toId) {
    ...
    if (dbFromIds.size() == pageSize) {
      ...
      while (totalCount > (page * pageSize)) {
        LinkedHashSet<Integer> tempUserId = clientRelationMapper
            .queryFromIdPage(toId, page * pageSize, pageSize);

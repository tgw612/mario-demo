package com.mario.service.api.filter;
...
/**
 * Created with IntelliJ IDEA. User: qiujingwang Date: 2018/9/10 Description:Dubbo 服务消费方远程调用过程拦截器
 * for 3.x
 */
@Service
@Slf4j
public class DubboFilter implements Filter {
  ...
  private void doLogBefore(Invoker<?> invoker, Invocation invocation) {
    log.info(BEFORE_LOG_MSG,
        getInitiationID(invocation), invoker.getInterface().getSimpleName(),
  ...
  private void doLogAfter(Invoker<?> invoker, Object response, Invocation invocation) {
    ...
    log.info(AFTER_LOG_MSG,
        getInitiationID(invocation), clazzName, methodName,
    ...
    if (args != null && args.length > 0) {
      for (Object arg : args) {
        if (arg instanceof CommonRequest) {
          ...
          initiationID = commonRequest.getInitiationID();
          break;
        }
      }
    }

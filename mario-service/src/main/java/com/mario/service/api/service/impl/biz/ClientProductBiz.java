...
public class ClientProductBiz {
  ...
  public List<ProductBO> queryHighProductPage(int currentPage, Integer highCommissionStatus) {
    ...
    return clientProductMapper
        .queryHighProductPage(productVo, (currentPage - 1) * pageSize, pageSize);
  }
   ...
   * @return
   */
  public List<ProductBO> queryHighProductPage(Integer highCommissionStatus, int startRecord,
      int recordSize) {
    //获取商品列表
    ProductVo productVo = DiscoverUtil.getProductVo();
    productVo.setHighCommissionStatus(highCommissionStatus);
  ...
  public List<ProductBO> queryHotProductPage(int currentPage, Integer hotProductStatus) {
    ...
    return clientProductMapper
        .queryHotProductPage(productVo, (currentPage - 1) * pageSize, pageSize);
  }
   ...
   * @return
   */
  public List<ProductBO> queryHotProductPage(Integer hotProductStatus, int startRecord,
      int recordSize) {
    //获取商品列表
    ProductVo productVo = DiscoverUtil.getProductVo();
    productVo.setHotProductStatus(hotProductStatus);
   ...
   * 通过二级类别获取相关商品
   */
  public List<ProductBO> queryProductListByCategory(Integer productCategory, int startRecord,
      int recordSize) {

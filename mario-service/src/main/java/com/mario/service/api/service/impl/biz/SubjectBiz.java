...
public class SubjectBiz {
  ...
  public CommonPageResult<SubjectBO> querySubjectPage(SubjectPageRequestDto request) {
    ...
    if (count > 0) {
      List<SubjectBO> subjectList = subjectMapper
          .querySubjectPage(subjectVo, PageUtils.getStartPage(request), request.getPageSize());

      //获取文章数量
      List<Long> subjectIdList = subjectList.parallelStream().map(SubjectBO::getSubjectId)
  ...
  public CommonPageResult<SubjectArticleResponse> querySubjectArticlePage(
...
//            SubjectBO subjectVO = subjectMapper.querySubjectInfo(subjectVo);
      ...
      subjectArticles.forEach(subject -> {
        try {
          ...
          SubjectArticleResponse response = objectMapper
              .readValue(value, SubjectArticleResponse.class);
          //设置关联商品
          List<ProductIdNameBO> productIdNameBOList = articleMapper
              .getArticleRelateProduct(subject.getArticleId(),
          ...
          List<SubjectInfoBO> relateSubjectName = articleMapper
              .getRelateSubjectName(subject.getArticleId(),
  ...
  public CommonResponse<ArticleHistorySubjectResponse> articleHistorySubject() {
    ...
    List<ArticleHistorySubjectResponse.HistorySubject> list = BeanCopyUtils
        .copyList(ArticleHistorySubjectResponse.HistorySubject.class, data);

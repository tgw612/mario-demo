...
public class ProductServiceBiz {
  ...
  public CommonPageResult<ProductDetailResponse> queryProductInfo(
      ProductESSearchRequestDto request) {
  ...
  public ProductInfoResponse queryProductInfo(String productNo) {
    ...
    if (dto.isSuccess() && ObjectUtil.isNotNull(dto.getResult())) {
      ...
      if (!CollectionUtils.isEmpty(productDetail.getActives())) {
        ...
        ProductDetailSearchResponse.ProductActive currentProductInfo = actives.stream()
            .filter(active -> {
              //发现server time为null 改为自用时间
  ...
  private void saveProductCount(ProductInfoResponse productInfo, ProductEntity entity) {
    ...
    articleCountEntity.setSort(
        sort == null || sort <= 0 || sort > discoverProperties.getMaxSort() ? discoverProperties
            .getMaxSort() : sort);
  ...
  public CommonResponse<CommonPageResult<ProductInfoResponse>> listProduct(
    ...
    List<ProductInfoBO> productInfos = productDao
        .listProduct(bo, PageUtils.getStartPage(request), request.getPageSize());
  ...
  private ProductInfoResponse convertToProductInfoResponse(ProductInfoBO info) {
    ...
    if (ObjectUtil.isNotNull(info.getHighCommissionStatus()) && 1 == info
        .getHighCommissionStatus()) {
    ...
    result.setArticleNums(relationMapper.selectProductRelateArticleCount(info.getProductId(),
  ...
  public CommonResponse<Boolean> deleteProductArticle(ProductDeleteArticleRequestDto request) {
    try {
      relationMapper
          .deleteProductArticle(RelationTypeEnum.GOODS_ARTICLE.getCode(), request.getArticleId(),
              request.getGoodsId());
      return ResponseManage.success(Boolean.TRUE);
    } catch (Exception e) {
      log.error("[{}]删除商品内文章失败, Exception:{}", SerialNo.getSerialNo(),
          ExceptionUtil.getAsString(e));
      return ResponseManage.fail("删除商品内文章失败");
    }
  }

  public CommonResponse<CommonPageResult<ProductArticleResponse>> queryProductArticlePage(
      ProductArticlePageRequestDto request) {
    try {
      ProductArticlePageRequestBO bo = BeanCopyUtils
          .copyProperties(ProductArticlePageRequestBO.class, request);
      ...
      if (count > 0) {
        ...
        productArticleBOList.forEach(productArticleBO -> {
          ProductArticleResponse data = BeanCopyUtils
              .copyProperties(ProductArticleResponse.class, productArticleBO);
          //设置articlelook和图片数量
          ...
          List<SubjectInfoBO> relateSubjectName = articleMapper
              .getRelateSubjectName(productArticleBO.getArticleId(),
          ...
          List<ProductIdNameBO> productIdNameBOList = articleMapper
              .getArticleRelateProduct(productArticleBO.getArticleId(),
                  RelationTypeEnum.GOODS_ARTICLE.getCode());
          List<ArticleRelateProduct> products = BeanCopyUtils
              .copyList(ArticleRelateProduct.class, productIdNameBOList);
      ...
      log.error("[{}]查询商品内文章失败, Exception:{}", SerialNo.getSerialNo(),
          ExceptionUtil.getAsString(e));
    ...
    for (int i = 1; i <= totalPage; i++) {
      ...
      productEntities.forEach(productEntity -> {
        try {
          ...
          log.error("[{}]更新productEntiry失败, Exception:{}", productEntity.getProductNo(),
              ExceptionUtil.getAsString(e));
        }
      });
    }
  ...
  public CommonResponse<Boolean> scheduleUpdateProductEntityV2() {
    ...
    for (int i = 1; i <= totalPage; i++) {
      ...
      List<String> productNoList = productEntities.stream().map(ProductEntity::getProductNo)
          .collect(Collectors.toList());
      CommonResponse<PageResponse<SearchProductResponse>> response = searchProductPage(
          productNoList);
      ...
      productEntities.forEach(productEntity -> {
        try {
          SearchProductResponse queryProductInfo = productNumInfoMap
              .get(productEntity.getProductNo());
          boolean flag = false;
          if (ObjectUtil.isNotNull(queryProductInfo)) {
  ...
  private boolean compareChangeProductField(ProductEntity productEntity,
      SearchProductResponse queryProductInfo, boolean flag) {
    if (ObjectUtil.isNotNull(productEntity.getProductName()) &&
        !productEntity.getProductName().equals(queryProductInfo.getName())) {
      productEntity.setProductName(queryProductInfo.getName());
      flag = true;
  ...
  private CommonResponse<PageResponse<SearchProductResponse>> searchProductPage(
    ...
    CommonResponse<PageResponse<SearchProductResponse>> response = esProductSearchService
        .searchByPage(productSearchRequest);
  ...
  private void updateProductHotField(ProductEntity productEntity,
      ProductInfoResponse queryProductInfo) {
    productEntity.setProductName(queryProductInfo.getName());
    productEntity.setMasterImg(queryProductInfo.getMasterImg());
    productEntity.setPrice(queryProductInfo.getPrice());
    productEntity.setCommission(queryProductInfo.getCommission());
  ...
  public CommonResponse<Boolean> updateReadShareCount() {
    ...
    Map<Integer, Long> productReadCountMap = productReadCount
        .getAll(countEntities.stream().map(entity -> {
          return Math.toIntExact(entity.getBizId());
        }).collect(Collectors.toSet()));
  ...
  /**
   * 1.查询出非高佣金/爆款的商品 2.判断关联文章数量,为0的 删除
   *
   * @return
   */
  public CommonResponse<Boolean> scheduleDeleteProductEntity() {
    ...
    if (!CollectionUtils.isEmpty(productIds)) {
      productIds.forEach(productId -> {
        int articleNum = relationMapper
            .selectProductRelateArticleCount(productId, RelationTypeEnum.GOODS_ARTICLE.getCode());
        if (articleNum == 0) {

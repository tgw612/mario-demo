package com.mario.service.api.aop;
...
import com.mario.service.api.exception.ExceptionManage;
...
/**
 * 服务实现切面处理 Modify by qiujingwang
 */
@Component
@Aspect
@Order(1)
  ...
  public Object methodHandle(ProceedingJoinPoint proceedingJoinPoint) throws Throwable {
    ...
    try {
      ...
      if (response.getErrorCode() == ErrorCodeEnum.ERR_INVALID_PARAM.getErrorCode() ||
        ...
        log.error("[{}][{}][{}] handling exception message is : [{}].", initiationID, className,
            methodName, e.getMessage());
      } else {
        log.error("[{}][{}][{}] handling exception message is : [{}] \n cause : [{}].",
            initiationID, className, methodName, e.getMessage(), ExceptionUtil.getAsString(e));
      }
    } finally {
            /*if(initiationID != null){
                SerialNo.clear();
      ...
      if (response.getErrorCode() == ErrorCodeEnum.SUCCEED_CODE.getErrorCode()) {
        log.info("[{}][{}][{}]Finish handling request, the response:[{}]", initiationID, className,
            methodName, response != null ? response.toString() : null);
      }
      return response;
    }
  }

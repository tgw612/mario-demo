...
import com.mario.service.api.service.impl.biz.MemberDiscoverBiz;
...
public class MemberDiscoverServiceImpl implements MemberDiscoverService {
  ...
  public CommonResponse<MemberDiscoverDetailResponse> detail(PrimarySeqRequest request) {
    MemberDiscoverDetailResponse detailResponse = JSON
        .parseObject(discoverCache.get(request.getId() + ""), MemberDiscoverDetailResponse.class);
  ...
  public CommonResponse<Integer> getProductShareCount(PrimaryIdRequest productIdRequest) {
    ...
    if (StringUtil.isNull(s)) {
      CommonResponse<Integer> productShareCount = memberDiscoverBiz
          .getProductShareCount(productIdRequest);
      discoverProductCache
          .put(productIdRequest.getId().toString(), productShareCount.getResult().toString(), 2,
              TimeUnit.MINUTES);
  ...
  public CommonResponse<Integer> findShareCount(FindShareCountRequest request) {
    ...
    if (StringUtil.isNull(s)) {
      CommonResponse<Integer> findShareCount2 = memberDiscoverBiz
          .findShareCount(request.getProductId());
      discoverShareProductCache
          .put(request.getProductId(), findShareCount2.getResult().toString(), 5, TimeUnit.MINUTES);

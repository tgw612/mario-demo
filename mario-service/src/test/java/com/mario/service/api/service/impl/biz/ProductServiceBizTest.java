...
//        request.setSort(sortRequest);
    CommonResponse<PageResponse<SearchProductResponse>> pageResponse = esProductSearchService
        .searchByPage(request);
    log.info("响应数据：{}", pageResponse);
    if (pageResponse != null && pageResponse.getResult() != null
        && pageResponse.getResult().getDatas() != null
        && pageResponse.getResult().getDatas().size() > 0) {
